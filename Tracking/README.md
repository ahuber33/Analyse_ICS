...
## 5. Convention de nommage des fichiers TIFF
...
| Paramètre | Valeur défaut | Description |
|---|---|---|
| `diameter_cellpose` | `11` | Diamètre estimé des lysosomes en pixels. Doit correspondre à la taille réelle dans l'image. |
...
| `top_hat_radius` | `10` | Rayon du disque (px) pour le top-hat blanc. Doit être supérieur au rayon des lysosomes. |
...
| `memory` | `4` | Nombre de frames pendant lesquelles une particule disparue est mémorisée avant abandon. |
...
| `Cellpose_Analyse_Track_GPU(...)` | `Include` | Top-hat + segmentation Cellpose + filtrage morphologique → DataFrame lysosomes. |
...
```python
device = Include_Lyso_Tracking.get_device()
```
...
2. **CLAHE** (`clip_limit`) : rehaussement de contraste local adaptatif — améliore la visibilité des lysosomes sans amplifier excessivement le bruit.
...
Segmente les lysosomes dans chaque ROI et retourne un DataFrame compatible TrackPy.
...
| `nombre_lysosomes_par_frame[_CNN].png` | Nombre de lysosomes détectés par frame au cours du temps. |
| `trajectoires_moyenne[_CNN].png` | Toutes les trajectoires filtrées superposées à l'image moyenne du stack. |
| `densite_lysosomes[_CNN].png` | Carte de densité KDE des positions des lysosomes sur l'image moyenne. |
...
| α < 1 | Sous-diffusion — lysosome confiné ou freiné (cage cytosquelette, encombrement). |
...
Cet histogramme (axe Y log) révèle la distribution des longueurs de pistes sur l'ensemble des particules détectées. Une bonne segmentation produit typiquement une décroissance approximativement exponentielle — la plupart des pistes courtes correspondent à des détections spurieuses ou des événements de clignotement, tandis que les pistes longues correspondent à des lysosomes réellement suivis sur de nombreuses frames.

Utiliser cette figure pour choisir `threshold_filtered` : le définir au-delà du coude de la distribution pour ne conserver que les lysosomes bien suivis.

---

### 8.4 Nombre de lysosomes par frame

Ce graphe temporel montre le nombre total de lysosomes détectés par frame. Il est utile pour :
...
La carte KDE agrège toutes les positions des lysosomes sur l'ensemble des frames. Les zones chaudes (régions claires sur la colormap `inferno`) indiquent des zones de concentration préférentielle ou de temps de résidence élevé. Cela peut refléter :
- Un **regroupement périnucléaire** (typique des lysosomes sous stress).
...
Le mode TrackPy seul (`flag_CNN=False`) repose sur la détection de maxima d'intensité et n'est pas bien adapté aux images denses où les lysosomes se chevauchent spatialement. Dans ce cas, le mode CNN+Cellpose offre une séparation d'instances significativement meilleure.
...
→ Réduire les tailles de batch dans `Tracking.py` :
```python
cnn_batch_size      = 128
...
**Trop ou trop peu de lysosomes comptés**  
→ Ajuster les paramètres Cellpose : augmenter `min_area` pour supprimer le bruit, diminuer `diameter_cellpose` si les lysosomes paraissent plus petits qu'attendu, ou affiner `min_circularity` pour filtrer plus agressivement les fragments de dendrites.

---

**`FileNotFoundError: soma_cnn_test.pth`**  
→ Le fichier de poids CNN doit être placé dans le **même dossier** que `Tracking.py`. Ne pas le renommer.
...
→ Augmenter `search_range` si les lysosomes bougent vite entre frames, augmenter `memory` s'ils disparaissent temporairement, ou baisser `threshold_filtered` pour conserver des trajectoires plus courtes.
